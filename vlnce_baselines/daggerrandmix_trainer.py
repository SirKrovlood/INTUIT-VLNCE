...
@baseline_registry.register_trainer(name="daggerrandmix")
class DaggerRandMixTrainer(BaseRLTrainer):
    ...
    def _update_dataset(self, data_it):
        ...
        with tqdm.tqdm(total=self.config.DAGGER.UPDATE_SIZE) as pbar, lmdb.open(
            ...
            while collected_eps < self.config.DAGGER.UPDATE_SIZE:
                ...
                goal_actions = batch["vln_oracle_action_sensor"]
                law_actions = batch["vln_law_action_sensor"]
                oracle_actions = torch.where(
                    torch.rand_like(goal_actions, dtype=torch.float) < beta,
                    law_actions,
                    goal_actions,
                )
                actions = torch.where(
                ...
                for i in range(self.envs.num_envs):
                    ...
                    episodes[i].append(
                        (
                            observations[i],
                            prev_actions[i].item(),
                            oracle_actions[i].item(),
                        )
                    )

                skips = oracle_actions.long() == -1
