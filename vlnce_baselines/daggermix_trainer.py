...
def collate_fn(batch):
    ...
    corrected_law_actions_batch = list(transposed[2])
    corrected_goal_actions_batch = list(transposed[3])
    ...
    for bid in range(B):
        ...
        corrected_law_actions_batch[bid] = _pad_helper(
            corrected_law_actions_batch[bid], max_traj_len
        )
        corrected_goal_actions_batch[bid] = _pad_helper(
            corrected_goal_actions_batch[bid], max_traj_len
        )
    ...
    corrected_law_actions_batch = torch.stack(corrected_law_actions_batch, dim=1)
    corrected_goal_actions_batch = torch.stack(corrected_goal_actions_batch, dim=1)
    weights_batch = torch.stack(weights_batch, dim=1)
    not_done_masks = torch.ones_like(corrected_law_actions_batch, dtype=torch.float)
    not_done_masks[0] = 0
    ...
    return (
        ...
        corrected_law_actions_batch,
        corrected_goal_actions_batch,
        weights_batch,
    )
...
class IWTrajectoryDataset(torch.utils.data.IterableDataset):
    ...
    def __next__(self):
        obs, prev_actions, law_actions, goal_actions = self._load_next()

        for k, v in obs.items():
            obs[k] = torch.from_numpy(np.copy(v))

        prev_actions = torch.from_numpy(np.copy(prev_actions))
        law_actions = torch.from_numpy(np.copy(law_actions))
        goal_actions = torch.from_numpy(np.copy(goal_actions))

        inflections = torch.cat(
            [
                torch.tensor([1], dtype=torch.long),
                (law_actions[1:] != law_actions[:-1]).long(),
            ]
        )

        return (obs, prev_actions, law_actions, goal_actions, self.inflec_weights[inflections])
...
@baseline_registry.register_trainer(name="daggermix")
class DaggerMixTrainer(BaseRLTrainer):
            ...
            while collected_eps < self.config.DAGGER.UPDATE_SIZE:
                ...
                for i in range(self.envs.num_envs):
                    if dones[i] and not skips[i]:
                        ...
                        transposed_ep = [
                            traj_obs,
                            np.array([step[1] for step in ep], dtype=np.int64), #previous actions
                            np.array([step[2] for step in ep], dtype=np.int64), #law actions
                            np.array([step[2] for step in ep], dtype=np.int64), #goal actions
                        ]
                        txn.put(
    ...
    def _update_agent(
        self, observations, prev_actions, not_done_masks, corrected_law_actions, corrected_goal_actions, weights
    ):
        T, N = corrected_law_actions.size()
        ...
        law_action_loss = F.cross_entropy(
            logits.permute(0, 2, 1), corrected_law_actions, reduction="none"
        )
        law_action_loss = ((weights * law_action_loss).sum(0) / weights.sum(0)).mean()

        goal_action_loss = F.cross_entropy(
            logits.permute(0, 2, 1), corrected_goal_actions, reduction="none"
        )
        goal_action_loss = ((weights * goal_action_loss).sum(0) / weights.sum(0)).mean()

        aux_mask = (weights > 0).view(-1)
        aux_loss = AuxLosses.reduce(aux_mask)

        action_loss = law_action_loss + goal_action_loss
                    ...
                    for batch in tqdm.tqdm(
                        ...
                        try:
                            loss, action_loss, aux_loss = self._update_agent(
                                ...
                                corrected_law_actions_batch.to(
                                    device=self.device, non_blocking=True
                                ),
                                corrected_goal_actions_batch.to(
                                    device=self.device, non_blocking=True
                                ),
                            ...
                            corrected_law_actions_batch = corrected_law_actions_batch.cpu()
                            corrected_goal_actions_batch =  corrected_goal_actions_batch.cpu()
                            ...
                            for i in range(not_done_masks.size(0)):
                                output = self._update_agent(
                                    ...
                                    corrected_law_actions_batch[i].to(
                                        device=self.device, non_blocking=True
                                    ),
                                    corrected_goal_actions_batch[i].to(
                                        device=self.device, non_blocking=True
                                    ),
                                    weights_batch[i].to(
                                        device=self.device, non_blocking=True
